...
import { Plus, Search, Phone, Mail, X, Building2, Calendar, User as UserIcon, Save, ArrowRight, Trash2, Tag, FileText } from 'lucide-react';
...
const STATUS_COLORS: Record<string, string> = {
  ...
  'Contacted': 'bg-indigo-50 text-indigo-700 border-indigo-200 ring-indigo-500/20',
  'Demo Booked': 'bg-purple-50 text-purple-700 border-purple-200 ring-purple-500/20',
  'Trial': 'bg-amber-50 text-amber-700 border-amber-200 ring-amber-500/20',
  'Closed Won': 'bg-emerald-50 text-emerald-700 border-emerald-200 ring-emerald-500/20',
  'Closed Lost': 'bg-slate-50 text-slate-600 border-slate-200 ring-slate-500/20',
};
    ...
    <div className="h-full flex flex-col bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex flex-wrap gap-6 items-center shrink-0">
        <div className="flex gap-8">
            ...
            <div>
                ...
                <p className="text-xl font-bold text-emerald-600">${totalPipeline.toLocaleString()}</p>
            </div>
        </div>
        ...
        <div className="ml-auto flex items-center gap-3">
            <div className="relative group">
                ...
                <input 
                    ...
                    className="pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none w-64 bg-white transition-all shadow-sm"
                />
            </div>
            <button 
                onClick={handleNew}
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 sticky top-0 z-10 text-xs font-bold text-slate-500 uppercase tracking-wider shadow-sm">
                <tr>
                    <th className="px-6 py-3 border-b border-slate-200 bg-slate-50">Company / Date</th>
                    <th className="px-6 py-3 border-b border-slate-200 bg-slate-50">Status</th>
                    <th className="px-6 py-3 border-b border-slate-200 bg-slate-50">Contact Person</th>
                    <th className="px-6 py-3 border-b border-slate-200 bg-slate-50">Deal Size</th>
                    <th className="px-6 py-3 border-b border-slate-200 bg-slate-50">Assigned To</th>
                    <th className="px-6 py-3 border-b border-slate-200 bg-slate-50 text-right"></th>
                </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 bg-white">
                {filteredLeads.map(lead => (
                    <tr key={lead.id} onClick={() => handleEdit(lead)} className="hover:bg-slate-50 transition-colors cursor-pointer group">
                        ...
                        <td className="px-6 py-4">
                            <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold border shadow-sm ${STATUS_COLORS[lead.status] || 'bg-slate-100'}`}>
                                {lead.status}
                            </span>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                            ...
                            <div className="text-[11px] text-emerald-600 font-medium bg-emerald-50 px-1.5 rounded w-fit mt-1">
                        ...
                        <td className="px-6 py-4 text-right">
                            <ArrowRight size={16} className="text-slate-300 group-hover:text-indigo-500 transition-colors inline-block"/>
                        </td>
                    </tr>
                ))}
            </tbody>
      ...
      {isSlideOverOpen && (
        <div className="fixed inset-0 z-[100] flex justify-end">
            ...
            {/* Panel */}
            <div className="relative w-full max-w-lg bg-white shadow-2xl h-full flex flex-col animate-slide-in-right border-l border-slate-200">
                ...
                <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-white z-10">
                    <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">Manage pipeline details and deal status.</p>
                    </div>
                    <button 
                        onClick={() => setIsSlideOverOpen(false)} 
                ...
                {/* Scrollable Content */}
                <form id="lead-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-8 bg-slate-50/50">
                    
                    {/* SECTION 1: Company */}
                    <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4">
                        <div className="flex items-center gap-2 text-sm font-bold text-indigo-600 uppercase tracking-wide border-b border-slate-100 pb-2 mb-2">
                ...
                {/* Footer */}
                <div className="p-5 border-t border-slate-200 bg-white flex gap-4 justify-between items-center z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                    {form.id ? (
                        <button 
                            type="button"
                    ...
                    <div className="flex gap-3">
                        <button 
                            type="button" 
                            onClick={() => setIsSlideOverOpen(false)} 
                            className="px-5 py-2.5 text-slate-600 font-bold hover:bg-slate-100 border border-slate-200 rounded-lg transition-all text-sm"
                        >
                            Cancel
                        </button>
                        <button 
