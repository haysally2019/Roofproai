...
import { Plus, Search, Phone, Mail, X, Building2, Calendar, User as UserIcon, Save, ArrowRight, Trash2 } from 'lucide-react';
...
interface Props {
  ...
  onDeleteLead?: (id: string) => void; // Added optional delete handler
}
    ...
    <div className="h-full flex flex-col bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      {isSlideOverOpen && (
        <>
            ...
            <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out border-l border-slate-200 flex flex-col animate-slide-in-right">
                ...
                <div className="p-4 border-t border-slate-200 bg-slate-50 flex gap-3 justify-between items-center">
                    {form.id ? (
                        <button onClick={handleDelete} className="text-red-500 hover:text-red-700 p-2 rounded hover:bg-red-50 transition-colors">
                            <Trash2 size={20}/>
                        </button>
                    ) : <div></div>}
                    
                    <div className="flex gap-3">
                        <button type="button" onClick={() => setIsSlideOverOpen(false)} className="px-4 py-2 text-slate-600 font-bold hover:bg-slate-200 rounded-lg transition-colors">Cancel</button>
                        <button onClick={handleSubmit} className="px-6 py-2 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 shadow-md flex items-center gap-2 transition-transform active:scale-95">
                            <Save size={18}/> Save Opportunity
                        </button>
                    </div>
                </div>
            </div>
