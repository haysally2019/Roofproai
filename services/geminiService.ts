...
export const getChatResponse = async (history: {role: string, parts: {text: string}[]}[], message: string) => {
  try {
    ...
    const chat = ai.chats.create({
      ...
      history: history,
      config: {
        systemInstruction: "You are RoofPro Bot, an AI assistant for a roofing insurance restoration CRM. You help users with claims, Xactimate questions, supplements, and roofing materials. Keep responses short and professional.",
      }
    });
...
export const draftClientEmail = async (clientName: string, topic: string, tone: 'professional' | 'friendly' | 'urgent'): Promise<string> => {
  try {
    const response = await ai.models.generateContent({
      ...
      contents: `Draft a short, ${tone} email to client ${clientName} about: ${topic}. Focus on the insurance claim process. Sign it as 'The RoofPro Team'.`,
    });
